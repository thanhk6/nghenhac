...
<div class="border-control">
    <div id="<%= ViewControlName%>$<%=CphName %>" <%if (!AloneMode){ %>style="cursor: move;" draggable="true" ondragstart="return dragStart(event)" <%} %>>
        <a href="javascript:void(0)" onclick="do_display('tbl_<%= ViewControlName%>');">
            <%= ViewControlID%> <%if (CurrentModule != null){ %> - <%= ((VSW.Lib.MVC.ModuleInfo)CurrentModule).Name %><%}else if (ErrorMode){ %> - Không tồn tại <%} %>
        </a>
    </div>
    ...
    <div class="form-body mt10" id="tbl_<%= ViewControlName%>" style="display: none;">
        ...
        <div class="form-group row" id="tr_<%= currentId%>">
            ...
            <div class="col-md-9">
                ...
                <select class="form-control" id="<%= currentId%>" name="<%= currentId%>">
                    ...
                    %>
                    <option <%if (currentValue == list[j].Value){%>selected<%} %> value="<%= list[j].Value%>"><%= list[j].Name%></option>
                    <%} %>
                </select>
                <%}else{ %>
        ...
        <div class="form-group row" id="tr_<%= ViewControlID%>_ViewLayout">
            ...
            <div class="col-md-9">
                <select class="form-control" id="<%= ViewControlID%>_ViewLayout" name="<%= ViewControlID%>_ViewLayout" <%if (layoutDefine != ""){ %>onchange="layout_change('<%= ViewControlID%>', '<%=layoutDefine %>', this.value)" <%} %>>
        ...
        <div class="cmd-control justify-content-end">
            ...
            <%if ((PosID & 1) != 1){ %>
            <a href="javascript:cp_update('<%= ViewControlID%>|up')" data-toggle="tooltip" data-placement="bottom" data-original-title="Chuyển lên trên"><i class="icon-arrow-up"></i></a>
            <%} %>
            <%if ((PosID & 4) != 4){ %>
            <a href="javascript:cp_update('<%= ViewControlID%>|down')" data-toggle="tooltip" data-placement="bottom" data-original-title="Chuyển xuống dưới"><i class="icon-arrow-down"></i></a>
            <%} %>

            <%} %>

            <%if (!AloneMode){ %>
            <a href="javascript:void(0)" onclick="sw_confirm('Thông báo !', 'Bạn chắc là mình muốn xóa chứ !', 'javascript:cp_update(\'<%= ViewControlID%>|delete\')')" data-toggle="tooltip" data-placement="bottom" data-original-title="Xóa điều khiển"><i class="icon-close"></i></a>
            <%} %>
        </div>
    </div>
</div>
