...
{
    [ModuleInfo(Name = "MO: Sản phẩm", Code = "MMp3", Order = 3)]
    public class MProductController : Controller
    {
        ...
        public void ActionIndex(MProductModel model)
        {
            ...
            var dbQuery = ModMp3Service.Instance.CreateQuery()
                .Select(o => new { o.ID, o.MenuID, o.Name, o.File, o.View, o.Code, o.Order, o.Activity, o.State })
                                    ...
                                    .Where(!string.IsNullOrEmpty(keyword), o => (o.Code.Contains(keyword)))
            ...
            //
            // 
            var CountProduct = ModMp3Service.Instance.CreateQuery()
                .Select(o => o.ID)
                                    .Where(o => o.Activity == true)
                                    .Where(State > 0, o => o.State == State)
                                    .Where(!string.IsNullOrEmpty(keyword), o => (o.Code.Contains(keyword)))
        ...
        public void ActionDetail(string endCode)
        {
            if (ViewPage.CurrentPage.MenuID > 0)
                MenuID = ViewPage.CurrentPage.MenuID;
            var item = ModMp3Service.Instance.CreateQuery()
           ...
           // if(item==null)re
            ...
            if (item != null)
            {
                ...
                //up view
                item.UpView();
                ViewBag.Other = ModMp3Service.Instance.CreateQuery()
                ...
                var listComment = ModCommentService.Instance.CreateQuery()
                   .Where(o => o.Activity == true && o.Mp3ID == item.ID && o.ParentID == 0);
