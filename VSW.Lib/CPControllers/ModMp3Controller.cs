...
namespace VSW.Lib.CPControllers
{
    [CPModuleInfo(Name = " danh sách nhạc",
        ...
        Activity = true,
        CssClass = "fa-cubes")]
    public class ModMp3Controller : CPController
    {
        public ModMp3Controller()
        {
            //khoi tao Service
            DataService = ModMp3Service.Instance;
            DataEntity = new ModMp3Entity();
            CheckPermissions = true;
        }
        public void ActionIndex(ModMp3Model model)
        {
            //sap xep tu dong
            ...
            var dbQuery = ModMp3Service.Instance.CreateQuery()
                                    ...
                                    .WhereIn(o => o.MenuID, WebMenuService.Instance.GetChildIDForCP("Mp3", model.MenuID, model.LangID))
                                   .WhereIn(model.AuthorID>0, o => o.AuthorID, WebMenuService.Instance.GetChildIDForCP("Author", model.AuthorID, model.LangID))
                                  .Take(model.PageSize)
                                    .OrderBy(orderBy)
        ...
        public void ActionAdd(ModMp3Model model)
        {
            if (model.RecordID > 0)
            {
                _item = ModMp3Service.Instance.GetByID(model.RecordID);
        ...
        public void ActionSave(ModMp3Model model)
        {
            if (ValidSave(model))
                SaveRedirect();
        }
        public void ActionApply(ModMp3Model model)
        {
            if (ValidSave(model))
                ApplyRedirect(model.RecordID, _item.ID);
        }
        public void ActionSaveNew(ModMp3Model model)
        {
            if (ValidSave(model))
        ...
        private ModMp3Entity _item;
        private bool ValidSave(ModMp3Model model)
        {
            ...
            {
                //save
                ModMp3Service.Instance.Save(_item);
                //update url
                ModCleanURLService.Instance.InsertOrUpdate(_item.Code, "Mp3", _item.ID, _item.MenuID, model.LangID);
                //cap nhat thuoc tinh
    ...
    public class ModMp3Model : DefaultModel
    {
