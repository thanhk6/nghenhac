...
namespace VSW.Lib.CPControllers
{
    public class SysPageController : CPController
    {
        ...
        private bool ValidSave(SysPageModel model)
        {
            ...
            {
                var parent = SysPageService.Instance.GetByID(model.ParentID);
                ...
                foreach (var t in model.Value.Split('\n'))
                {
                    ...
                    var menu = WebMenuService.Instance.CreateQuery()
                                        .Where(o => o.Code == _item.Code && o.ParentID == parent.MenuID)
                                        .ToSingle();
                    ...
                    _item.ModuleCode = "M" + model.Type;

                    string file = model.Type + ".Master";

                    var template = SysTemplateService.Instance.CreateQuery()
                                                .Where(o => o.File == file && o.LangID == model.LangID)
                    ...
                    _item.Code += (!string.IsNullOrEmpty(model.Extension) ? ("-" + model.Extension) : string.Empty);
